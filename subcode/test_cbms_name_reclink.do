...
* odbc query "phil"
* odbc load, exec("SELECT A.conacct, B.brgy, B.pop_2007 FROM  bnear AS A JOIN barangay AS B ON A.OGC_FID=B.OGC_FID WHERE municipali=='PASAY CITY'") clear
...
prog define hnum_gen_cbms
	...
	replace `var' = subinstr(`var',"-","",.)
	stnd_specialchar `var',  patpath("${subcode}")
	}
...
save "${temp}rlink_11c.dta", replace
...
duplicates drop conacct, force
	...
	merge m:1 conacct using "${temp}rlink_11c.dta"
	keep if _merge==3
	drop _merge
...
destring amount hhsize hhemp age job wrs_exp_extra, replace

g amount_tot= amount
replace amount_tot = amount+wrs_exp_extra if wrs_exp_extra!=. 

keep conacct cbms_id amount amount_tot hhsize hhemp age job match 

ren (amount amount_tot hhsize hhemp age job) (amount_paws amount_tot_paws hhsize_paws hhemp_paws age_paws job_paws)
...
replace amount_tot_paws = . if amount_tot_paws>5000
replace amount_paws=. if amount_paws>5000
* corr hhsize hsize if match>.9


corr bill amount_paws
corr bill amount_tot_paws

corr bill amount_paws if match>.9
corr bill amount_tot_paws if match>.9


corr hhsize_paws hhsize if match==1
corr hhsize_paws hhsize if match==1



corr bill amount_paws if match==1
...
reg bill hhsize 
reg bill hhsize_paws if match>.9


reg amount_paws hhsize  if match>.9
reg amount_paws hhsize_paws if match>.9



reg bill inc if match>.9
