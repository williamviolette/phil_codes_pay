...
r_low  = 0.00;
r_high = 0.2;
beta   = inv(1+0.02+r_high);              % subjective discount factor 
% beta   = inv(1+0.21);              % subjective discount factor 


pr_h=.9;
pr_l=.1;
prob   = [ pr_h pr_h pr_l pr_l; 
           pr_h pr_h pr_l pr_l;
           pr_h pr_h pr_l pr_l;
           pr_h pr_h pr_l pr_l]./2 ;   % prob(i,j) = probability (Y(t+1)=Yj | Y(t) = Yi)
...
[util1,util2,util3,util4,w1,w2,w3,w4] = ...
    u_w1loan(A,Aprime,alpha,p,r_high,r_low,Y_high,Y_low,lambda_high,lambda_low);
...
for i = 1:n-1
    ...
    u_w1loan(Athis,Aprime,alpha,p,r_high,r_low,Y_high,Y_low,lambda_high,lambda_low);
