...
beta   = inv(1+0.15);              % subjective discount factor 
...
Y_high = 20000 ;               % high value for income
Y_low  = 20000 ;           % low value for income
...
[util1h,util2h,util3h,w1h,w2h,w3h] = ...
    u_w2loans(A,Aprime,alpha,p,r_high,r_low,Y_high,lambda_high);

[util1l,util2l,util3l,w1l,w2l,w3l] = ...
    u_w2loans(A,Aprime,alpha,p,r_high,r_low,Y_low,lambda_low);
...
n = 50000;                   % number of periods to simulate
s0 = 1;                    % initial state 
states   = zeros(n-1,2);
...
for i = 1:n-1
    ...
    [~,~,~,w1h,w2h,w3h] = ...
    u_w2loans(Athis,Aprime,alpha,p,r_high,r_low,Y_high,lambda_high);

    [~,~,~,w1l,w2l,w3l] = ...
    u_w2loans(Athis,Aprime,alpha,p,r_high,r_low,Y_low,lambda_low);
