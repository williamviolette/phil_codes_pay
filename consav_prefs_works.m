...
Y_high = 1.5;               % high value for income
Y_low  = 1.5;           % low value for income
rho_high=1.1;
rho_low =1.8;
...
cons1 = A + Y_high-Aprime/(1+r);
cons2 = A + Y_low-Aprime/(1+r);
cons3 = A + Y_high-Aprime0/(1+r);
cons4 = A + Y_low-Aprime0/(1+r);
...
util1 =  rho_low.*log(cons1);
util2 =  rho_high.*log(cons2);
util3 =  rho_low.*log(cons3);
util4 =  rho_high.*log(cons4);

%util1 =  ((cons1).^(1-rho_high)-1)./(1-rho_high) ;
%util2 =  ((cons2).^(1-rho_low)-1)./(1-rho_low) ;
%util3 =  ((cons3).^(1-rho_high)-1)./(1-rho_high) ;
%util4 =  ((cons4).^(1-rho_low)-1)./(1-rho_low) ;
...
for i=6:size(states,1)-6
   if states(i-5,2)<=2 && states(i-4,2)<=2 && states(i-3,2)<=2 && states(i-2,2)<=2 && states(i-1,2)<=2 && states(i,2)>=3
      C1=[C1 C(i-5:i+1,1)]; 
   end 
end
