...
This paper incorporates delayed bill payments into household consumption and savings decisions in order to evaluate the welfare effects of counterfactual payment policies such as prepaid metering.  This paper studies a large piped water utility in Manila, Philippines where detailed administrative data record household consumption, bill payments, and disconnections for nonpayment.  Manila's regulation of nonpayment and disconnection is similar to policies in many other middle and high-income countries (\cite{marques2023water}).  In Manila, unpaid water bills average 5.5\% of household income, exceeding most other sources of short-term credit.\footnote{On average, short-term loans, which are mainly from moneylenders and microcredit agencies, only equal about 2.5\% of household income according to the 2014 Consumer Finance Survey.}  Descriptive evidence suggests that households time their bill payments to smooth their consumption when they face sudden income changes.  Out of each 100 PhP increase in income, households spend around 1 PhP on their unpaid water bills without changing their water consumption.\footnote{``PhP'' refers to Philippine Pesos where 45 PhP = 1 USD.}

Manila provides a lower-middle income country setting with universal metering, systematic enforcement of arrears and disconnections, as well as large investments in service quality (\cite{wu2008tale}).  These features allow for precisely measuring consumer behavior at high frequency.  Although many developing cities currently operate with lower service quality levels and weaker enforcement capacity (\cite{kingdom2006challenge}), the Manila experience is likely informative for cities planning similar, large infrastructure investments (\cite{WEF2025WaterGap}).
...
Disconnection regulations in Manila are similar to those in many other middle and high-income countries.  Across 12 countries reviewed by \cite{marques2023water}, the majority of water utility regulators also allow service disconnection for nonpayment, enforce how utilities must warn their customers of upcoming disconnections, and do not require utilities to maintain a minimum vital water flow.  Other countries also impose caps on late penalties ranging from 1.5 to 11\% of unpaid balances.\footnote{Caps on late fees equal 1.5\% of monthly unpaid balances in New York State to 11\% in Jerusalem (\cite{nycrr_11_15} and \cite{shavit_krause_polemio_2024}).   Other cities in the Philippines are permitted to charge penalties of up to 10\% of unpaid balances (\cite{mnwd_guidelines_bills_payments} and \cite{ppcwater_ur_billing}).} As a result, the payment and disconnection patterns observed in Manila are broadly comparable to those among other large utilities serving middle to low-income populations.\footnote{In a case study in Mexico, 30\% of consumers did not pay their water bills on time (\cite{marques2023water}).  Likewise, among low-income consumers of two large electricity utilities in the United States, 14 to 26\% had balances past due, and among all customers, around 4\% were served disconnection notices, 20\% received late fees, and 1\% were disconnected due to nonpayment (\cite{cicala2021incidence}).  In South Africa, 20\% of Cape Town electricity consumers were observed disconnected for at least one month likely due to nonpayment (\cite{jack2020charging}).} 

% \cite{marques2023water} document default rates ranging from around 1\% in Europe to 6.5\% in Latin America. 
...
The last counterfactuals consider using penalty fees to reduce billing delinquency. For example, other water providers in the Philippines impose late penalties of around 10\% of total unpaid balances.\footnote{See \cite{mnwd_guidelines_bills_payments} and \cite{ppcwater_ur_billing}.}

Column (4) of Tables~\ref{table:countercosts} and \ref{table:counter} considers a fixed late penalty of 123.5 PhP while the household is connected and delinquent.  This penalty is calibrated to equal 10\% of the average observed unpaid balance.   Column (5) of Tables~\ref{table:countercosts} and \ref{table:counter} imposes a fixed monthly interest rate of 4.9\% on unpaid water bills that accumulate while the household is connected and delinquent.  This percentage is calibrated to reflect the fact that households are on average two months in arrears, so the penalty compounded over two months would reach 10\% of the unpaid balance.
% \footnote{This policy imposes an additional fee of $10\% \times 1{,}235 = 123.5$ PhP each month that a household carries a negative water balance ($B_{t+1}<0$) and remains connected.}
