...
The next counterfactual considers a late penalty equal to 10\% of the average unpaid balance.  This policy imposes an additional fee of $0.1 \times 1{,}235 = 123.5$ PhP each month that a household carries a negative water balance ($B_{t+1}<0$).  This penalty is modeled as an extra deduction from household income in months where the household chooses to carry unpaid bills into the next period.  Column~(4) of Table~\ref{table:countercosts} summarizes the cost changes under this policy, and Column~(4) of Table~\ref{table:counter} provides the corresponding household outcomes.  The compensating variation in Column~(4) measures the welfare impact of the late penalty after adjusting prices to maintain revenue neutrality.
...
\midrule   
Assumed Preferences\\
\hspace{1em}Discount Rate ($\delta$)  & .005  & .027 & .0025  & .005 & .005     \\
...
The main analysis approximates the tariff structure using a linear regression of average price on monthly usage, yielding a constant term $p_1$ and a slope $p_2$.  This appendix considers a robustness check that eliminates the curvature of prices by setting $p_2 = 0$ and $p_1 = 27.8$, so that the marginal price is constant at 27.8 PhP per m3.  Table~\ref{table:counter_linp} reproduces the counterfactual household outcomes from Table~\ref{table:counter} under this linear price assumption.

\begin{table}[H]
\centering
\caption{Household Outcomes under Counterfactual Policies with Linear Prices}\label{table:counter_linp}
% \resizebox{\columnwidth}{!}{%
\begin{threeparttable}
\begin{tabular}{@{}l*{1}{EEEEE}@{}}
\toprule
...
\item
\footnotesize
This table reproduces Table~\ref{table:counter} under the assumption of linear prices with $p_1 = 27.8$ and $p_2 = 0$.  All measures are per average household-month. Compensating variation measures the change in monthly income in PhP needed to make households indifferent between the current scenario and the counterfactual scenarios.  Standard borrowing measures the average amount of borrowing from the standard asset in PhP.  Bill and outstanding balance are in PhP.  Column (4) applies a late penalty of $0.1 \times 1{,}235 = 123.5$ PhP per month when $B_{t+1}<0$.  Column (5) applies a 4.9\% monthly interest rate on unpaid bills so that $B_{t+1}$ becomes $B_{t+1}/(1+0.049)$.    45 PhP = 1 USD
\end{tablenotes}
\end{threeparttable}
% }
\end{table}
