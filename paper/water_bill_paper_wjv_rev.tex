...
\subsection{Late Penalty}

The next counterfactual considers a late penalty equal to 10\% of the average unpaid balance, which is consistent with billing policies among other public water providers in the Philippines.\footnote{This policy imposes an additional fee of $10\% \times 1{,}235 = 123.5$ PhP each month that a household carries a negative water balance ($B_{t+1}<0$) and remains connected. See \cite{mnwd_guidelines_bills_payments} and \cite{ppcwater_ur_billing} for examples.}

.  This penalty is modeled as an extra deduction from household income in months where the household chooses to carry unpaid bills into the next period.  

Column~(4) of Table~\ref{table:countercosts} summarizes the cost changes under this policy, and Column~(4) of Table~\ref{table:counter} provides the corresponding household outcomes.  The compensating variation in Column~(4) measures the welfare impact of the late penalty after adjusting prices to maintain revenue neutrality.

\subsection{Interest Rate on Unpaid Bills}

The final counterfactual considers imposing a fixed monthly interest rate of 4.9\% on unpaid water bills.  Under this policy, unpaid balances are effectively discounted each month so that $B_{t+1}$ becomes $B_{t+1}/(1+0.049)$.  This interest charge raises the cost of carrying unpaid bills and reduces the implicit credit subsidy that the utility provides to delinquent households.  Column~(5) of Table~\ref{table:countercosts} summarizes the cost changes under this interest rate policy, and Column~(5) of Table~\ref{table:counter} provides the corresponding household outcomes.  The compensating variation in Column~(5) measures the welfare impact after adjusting prices to maintain revenue neutrality.
...
\subsection{Linear Prices}\label{appendix:linearprices}

The main analysis approximates the tariff structure using a linear regression of average price on monthly usage, yielding a constant term $p_1$ and a slope $p_2$.  This appendix considers a robustness check that eliminates the curvature of prices by setting $p_2 = 0$ while keeping $p_1$ at its estimated value.  Table~\ref{table:counter_linp} reproduces the counterfactual household outcomes from Table~\ref{table:counter} under this linear price assumption.
