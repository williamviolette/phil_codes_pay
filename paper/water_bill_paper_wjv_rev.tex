...
Utility disconnections for nonpayment are common across both developing and developed countries.  In the United States, \cite{cicala2021incidence} finds that the odds of being disconnected from electricity service are four times higher in Black and Hispanic zip codes, and \cite{franklin2017} document that low-income and minority communities are disproportionately affected by shutoffs of electricity and water service.  Many US states restrict disconnections during extreme weather to protect public health (\cite{liheap}), and utilities around the world suspended disconnections during the COVID-19 pandemic (\cite{bufordcampbell2020}; \cite{domingo2020}).  A cross-country comparison by \cite{marques2023water} documents that water shutoffs for nonpayment are explicitly forbidden in the United Kingdom, Australia, and France, while many other medium- and high-income countries permit disconnection as a debt recovery tool.  In developing countries, responses to nonpayment vary widely: in South Africa, high rates of nonpayment for electricity motivated the widespread adoption of prepaid meters (\cite{jack2020charging}; \cite{szabo2015reducing}); in Colombia, \cite{mcrae2015infrastructure} documents how utilities underinvest in infrastructure in areas with high nonpayment; and prepaid water meters are now in use in more than 20 African countries as utilities seek to improve cost recovery (\cite{heymans2014limits}).  More broadly, \cite{burgess2020consequences} describe a ``vicious circle'' in which tolerated nonpayment undermines utility finances, leading to supply rationing and further erosion of payment incentives---a dynamic that extends naturally to water.

Policies regarding interest and penalties on utility arrears vary considerably.  In many countries, water is treated as a basic human right (\cite{unga2010water}), and utilities are prohibited or restricted from charging interest on late payments.  In the Philippines, the water utility in Manila cannot charge interest on outstanding bills.  However, other Philippine water providers impose late penalty surcharges, typically around 10\% of the outstanding amount (\cite{mnwd_guidelines_bills_payments}; \cite{ppcwater_ur_billing}).  In developed countries, practices are mixed: some utilities charge interest or late fees on overdue accounts while others face regulatory restrictions, particularly for essential services like water (\cite{hoque2013state}; \cite{marques2023water}).  The absence of interest charges on arrears effectively makes delayed bill payments an interest-free loan from the utility to the household---an implicit lending role that is rarely recognized in the design of utility regulation (\cite{laffont2005regulation}).  Yet the utility may have a natural comparative advantage in extending credit relative to formal lenders: it can use the threat of disconnection as implicit collateral, its existing billing infrastructure lowers screening and monitoring costs, and its broad customer base allows it to diversify default risk.

Several features make Manila a useful setting for studying how billing policy affects household welfare.  First, households face very limited access to formal credit, with the most common lending sources---moneylenders and microfinance groups---charging average monthly interest rates around 9.5\%.  The sharp contrast between high market borrowing rates and zero interest on utility arrears makes the credit value of delayed payments especially large in this setting.  \cite{devoto2012happiness} demonstrate that relaxing credit constraints for water connections in Morocco leads to high take-up and large welfare gains, underscoring the importance of credit access for water-related decisions among low-income households.  Second, the water utility tolerates substantial delinquency before disconnecting households, and detailed administrative billing records allow for observing household consumption, payments, and disconnections at high frequency.  Third, Manila's water sector was privatized in 1997, splitting Metro Manila between two concessionaires operating under identical regulatory conditions (\cite{wu2008tale}).  This public-private regulatory structure adjusts prices to ensure cost recovery---and over the sample period the utility expanded connections from 900,000 to 1,500,000 while maintaining high customer satisfaction---providing a natural framework for revenue-neutral policy comparisons in a setting where investment incentives appear to be functioning well.  The core mechanism identified in this paper---households timing bill payments to smooth consumption---is likely to arise wherever credit-constrained households face utilities that tolerate delinquency, a common pattern in both developing and developed countries (\cite{burgess2020consequences}).  The quantitative magnitudes will depend on local credit conditions, enforcement practices, and the share of household budgets devoted to utility payments.
...
% \input{tables/descriptives_new_all}
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
...
Lending Costs  & \input{tables/lend_cost_sum_reg_b}  & \input{tables/lend_cost_sum_nl_b}  & \input{tables/lend_cost_sum_hf_b}  & \input{tables_new/lend_cost_sum_lp_b}  & \input{tables_new/lend_cost_sum_ir_b}  \\
...
Warning Costs & \input{tables/visit_cost_reg_b} & \input{tables/visit_cost_nl_b}    &   \input{tables/visit_cost_hf_b}  & \input{tables_new/visit_cost_lp_b}  & \input{tables_new/visit_cost_ir_b}  \\
Pumping Costs  &  \input{tables/mc_cost_reg_b}  & \input{tables/mc_cost_nl_b}    &   \input{tables/mc_cost_hf_b}  & \input{tables_new/mc_cost_lp_b}  & \input{tables_new/mc_cost_ir_b}  \\
Fixed Cost &  \input{tables/rev_goal_reg_b}  &  \input{tables/rev_goal_reg_b}     &   \input{tables/rev_goal_reg_b}  &  \input{tables/rev_goal_reg_b}  &  \input{tables/rev_goal_reg_b}  \\
Price per m3  &  \input{tables/p1_reg_b} & \input{tables/p1_nl_b}    &  \input{tables/p1_hf_b}  & \input{tables_new/p1_lp_b}  & \input{tables_new/p1_ir_b}  \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item
...
\toprule
 ...
 & Current & Prepaid Metering & 50\% Less Enforcement  & Late Penalty & 4.9\% Interest  \\
\midrule
Compensating Variation &   & \input{tables/cv_comp_nl_b}  & \input{tables/cv_comp_hf_b}  & \input{tables_new/cv_comp_lp_b}  & \input{tables_new/cv_comp_ir_b}  \\
Consumption (m3)	 &   \input{tables/cons_reg_b} & \input{tables/cons_nl_b}    &  \input{tables/cons_hf_b}  & \input{tables_new/cons_lp_b}    &  \input{tables_new/cons_ir_b} \\
Bill			 &   \input{tables/cons_val_reg_b} & \input{tables/cons_val_nl_b}    &  \input{tables/cons_val_hf_b}  & \input{tables_new/cons_val_lp_b}    &  \input{tables_new/cons_val_ir_b} \\
Unpaid Balance & \input{tables/debt_reg_b} & \input{tables/debt_nl_b}    &  \input{tables/debt_hf_b}  & \input{tables_new/debt_lp_b}    &  \input{tables_new/debt_ir_b}  \\
Standard Borrowing & \input{tables/Aborr_abs_reg_b} & \input{tables/Aborr_abs_nl_b}    &  \input{tables/Aborr_abs_hf_b}  & \input{tables_new/Aborr_abs_lp_b}    &  \input{tables_new/Aborr_abs_ir_b}  \\
...
Column (4) of Tables~\ref{table:countercosts} and Table~\ref{table:counter} considers a fixed late penalty calibrated to 10\% of the average unpaid balance.\footnote{This policy imposes an additional fee of $10\% \times 1{,}235 = 123.5$ PhP each month that a household carries a negative water balance ($B_{t+1}<0$) and remains connected.} Column (5) of Tables~\ref{table:countercosts} and Table~\ref{table:counter} imposes a fixed monthly interest rate of 4.9\% on unpaid water bills that accumulate while the household is connected, calibrated to reflect the fact that households are on average two months in arrears.

These policies have similar effects.  Both leave households roughly indifferent to the current equilibrium.  Households consume borrow much less from their unpaid balances.  Cost savings from lower unpaid balances are passed onto households in the form of slightly lower water prices.  
...
The dataset for the main analysis focuses on months including and after March, 2010 because these months do not include gaps in payment and billing records (except that all data are missing for June, 2014).  Excluding data before March, 2010 brings the final sample size to \input{tables/full_obs}household-months.
...
Keep months after March, 2010 (with complete records)  &            &   \input{tables/datedrop}    \\
Final sample & \input{tables/full_obs} & \\
\bottomrule
\end{tabular}
}
\end{table}
...
\hspace{1em}Late Penalty & \input{tables_new/cv_comp_lp_b} & \input{tables_new/cv_comp_lp_bhigh} & \input{tables_new/cv_comp_lp_blow} & \input{tables_new/cv_comp_lp_chigh} & \input{tables_new/cv_comp_lp_clow}    \\
\hspace{1em}4.9\% Interest Rate & \input{tables_new/cv_comp_ir_b} & \input{tables_new/cv_comp_ir_bhigh} & \input{tables_new/cv_comp_ir_blow} & \input{tables_new/cv_comp_ir_chigh} & \input{tables_new/cv_comp_ir_clow}    \\
% &   (\input{tables/est_sd_pc_b}\unskip) \\[.8em] % Households &  \input{tables/est_hhs} \\ % Household-Months &  \input{tables/est_obs} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item 
