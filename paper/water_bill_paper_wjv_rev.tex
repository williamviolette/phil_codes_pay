...
Column (4) of Tables~\ref{table:countercosts} and Table~\ref{table:counter} considers a fixed late penalty of 123.5 PhP while the household is connected.  This penalty is calibrated to equal 10\% of the average observed, unpaid balance.   Column (5) of Tables~\ref{table:countercosts} and Table~\ref{table:counter} imposes a fixed monthly interest rate of 4.9\% on unpaid water bills that accumulate while the household is connected.  This percentage is calibrated to reflect the fact that households are on average two months in arrears, so the penalty would compound over two months to reach 10\% of the unpaid balance.
