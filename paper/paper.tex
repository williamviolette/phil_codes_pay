...
\toprule
 ...
 \toprule
\input{tables/table_panel_analysis}
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
...
u(w_{\tau},x_{\tau}) = \alpha \, log(w_{\tau}) + x_{\tau}
\end{split} 
\end{align}
where households have a rate of time preference $\delta \in (0,1)$.  Households have quasi-linear preferences over water consumption, $w_{t}$, and a bundle of other goods, $x_{t}$, given by $\alpha \in (0,1)$.  Quasi-linear preferences assume that income has no effect on consumption consistent with descriptive evidence that fluctuations in household employment are uncorrelated with water spending.  Households are assumed to have constant relative risk aversion for consumption across months given by $\gamma$.
