...
% NEED TO WRITE !! PLEASE !!! HERE !! WE GO !!!
...
\bottomrule \\ [-.8em]
\multicolumn{7}{c}{Total Households: \input{tables/total_hhs_all}  Obs. per Household: \input{tables/obs_per_hh_all} Total Obs.: \input{tables/total_obs_all}}
\end{tabular}
\end{table}
...
Mean Usage (m3) & \input{tables/c_h} & \input{tables/c_nl}  & \input{tables/c_ppe} & \input{tables/c_pp} \\
 &         &           &         &  \\
%Adjustments to stay revenue neutral  & & \\
Price Intercept $p_1$ (PhP/m3) & \input{tables/p_int} & &  \input{tables/p_int_ppe} & \input{tables/p_int_pp} \\
