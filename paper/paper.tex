...
%%%% NEED SOMETHING SURPRISING HERE!!
...
% and evaluate the welfare effects of a popular proposal to eliminate delinquency.  I find that households are willing to pay an 8\% premium on their water bill to have bill flexibility.  Implementing prepaid ....
 ...
 % This framework also develops a method to estimate credit constraints with utility billing data.



% prob don't get caught = 1 - .04  = sum ( .96^k )  = 1/(1-.96) = 25, actually works pretty well for ballparking...
...
%While microfinance and peer-to-peer lending have received substantial research and policy attention (\cite{morduch1999microfinance}), studies have yet to examine the welfare implications of firms delaying payments despite the capacity of this practice to reach broad segments of the population.  

%While microfinance and peer-to-peer lending have received substantial research and policy attention (\cite{morduch1999microfinance}), studies have yet to examine the welfare implications of offering credit through deferred payments.
...
% Since public utilities cover broad populations and take up at least 5\% of household incomes, delayed payments to public utilities likely account for an economically significant share of the total credit available to households in developing countries.\footnote{\cite{komives2006distributional} find that households spend 1-2\% of their incomes on water and 4\% on electricity.} 

%\footnote{For example, \cite{komives2006distributional} find revenue recovery rates ranging for utilities ranging from 16\% to 98\% among Eastern European Countries.}
%For example, in Manila, total outstanding balances for water bills are double those for credit cards. 

%This source of credit is able to reach broad populations 
%likely to be economically substantial since public utilities cover broad populations and 

% Yet, since prices are often regulated to recoup costs, defaulting consumers may lead to price increases for all consumers, distorting their future consumption.  Moreover, linking credit to consumption may incentivize overconsumption.  

% As monopoly providers,  % instead of requiring upfront collateral. 
...
% because as monopoly providers, utilities are able to threaten disconnection from service to enforce payments,   and.  

% (\cite{laffont2005regulation})
%Also, since governments often regulate utility prices to cover their production costs, any credit risk 

% Policymakers worry that high rates of delinquency weaken incentives for utilities to invest in infrastructure, especially in areas with low-income populations (\cite{mcrae2015infrastructure}, \cite{szabo2015reducing}).

% NEED TO WRITE !! PLEASE !!! HERE !! WE GO !!!    Low interests rates on postponed payments lead 
% , which mimics strategies used by many public utilities to ensure payment.  
% This paper incorporates delayed payments for goods into household consumption and savings decisions in order to (1) estimate credit constraints facing households and (2) evaluate the welfare effects of counterfactual billing policies.  

This paper incorporates delayed payments to public utilities into household consumption and savings decisions in order to evaluate the welfare effects of offering credit through public utilities.  I build a dynamic model where each period, households have the option of borrowing with a costly asset as well as with delayed payments to a public utility.  Low interest rates on delayed payments induce households to increase their borrowing by overconsuming utility services.  The extent to which households value payment flexibility depends on the borrowing costs of other assets, which are often difficult to measure in traditional survey data. %\footnote{Survey data often record current loans for households instead of reporting the quality of available credit.}
...
% Put this part in here!?

% These estimates allow me to examine how much consumer welfare declines when 

% To measure how much households value credit from delayed bills, I use these estimates to examine how consumer welfare declines when households are required to pay their water bills on time.  I find that households would be billing to pay an 8\% premium on their average water bill in order to keep billing flexibility. 

% These estimates imply that households would require a monthly payment equal to 

% would be willing to give up 8\% of their total water bill in order to keep their billing flexiblity .

%are consistent with this descriptive evidence, producing a large predicted interest rate for borrowing.  
%In Manila, households have low access to formal credit with only 3.9\% having credit cards and 18.7\% having bank accounts.\footnote{Statistics are from the Philippines 2014 Consumer Finance Survey.} 
% The estimates imply that households would require a monthly payment equal to 8\% of their current water bills in order to agree to always paying their bills on time.

These estimates also allow me to simulate several counterfactual policies.  First, when households are required to pay their bills on time (holding all else equal), consumer welfare drops by \input{tables/U_nl_abs}PhP (or \input{tables/U_nl_abs_usd}USD) per household-month, which is equal to \input{tables/U_nl_per}\unskip\% of an average water bill.  Usage also declines since households no longer have an incentive to overconsume water.  Second, I simulate a policy that (1) eliminates delayed payments by charging a high interest rate on unpaid bills, (2) eliminates default risk by preventing households from leaving large outstanding balances when they permanently disconnect, and (3) adjusts prices to ensure that the water utility remains revenue-neutral (consistent with regulation in Manila).\footnote{\cite{laffont2005regulation} discusses economic reasons for these regulatory structures which are common in both developed and developing country settings.}  This policy reduces social welfare by \input{tables/U_ppe_abs}PhP (or \input{tables/U_ppe_abs_usd}USD) per household-month.  Finally, I use this framework to evaluate implementing prepaid meters in this context.  Prepaid meters require steep increases in prices to cover their installation and maintenance costs.  As a result, prepaid meters produce even larger reductions in social welfare on the order of \input{tables/U_pp_abs}PhP (or \input{tables/U_pp_abs_usd}USD) per household-month.  These results suggest that popular policies to reduce nonpayment may be welfare reducing by limiting consumption smoothing.


% Technologies that eliminate delayed payments have recently dominated utility policy discussions.  Policymakers worry that high rates of delinquency weaken incentives for utilities to invest in infrastructure, especially in areas with low-income populations (\cite{mcrae2015infrastructure}, \cite{szabo2015reducing}).  Both researchers and policymakers recommend new prepaid metering technologies, which require upfront payment before releasing any services, as effective strategies to prevent nonpayment.\footnote{\cite{kojima2016making} and \cite{heymans2014limits} are broadly supportive of prepaid metering for electricity and water utilities (respectively).}  \cite{jack2016charging} find that prepaid meters for electricity in South Africa substantially increase utility revenues.\footnote{\cite{jack2015pay} also argue that prepaid meters may help savings constrained consumers by allowing households to pay small increments each month.}  Yet, existing analyses have yet to consider how these technologies may impact households' welfare by reducing their ability to smooth consumption.

% In Manila as well as many other cities, a government regulator determines prices for the water utility in order to exactly cover its costs.\footnote{In fact, water prices in Manila fluctuate monthly according to changes in the financial returns of the water company's assets.  \cite{laffont2005regulation} discusses economic reasons for these regulatory structures.}

% In each period, each household faces some probability of losing access to future consumption until they pay their full outstanding balance. This feature provides a framework for estimating credit constraints from utility billing data.  Utilities often record the timing of visits when they attempt to disconnect nonpaying households.  Without credit constraints, disconnections would never take place during these visits because households would always take out low-cost loans to pay their balances.  Therefore, the frequency and duration of disconnections in response to these visits, especially among households with large outstanding balances, help to identify the magnitude of credit constraints.

%  \input{tables/est_irate_annual_per}\unskip\% 
% The structural estimation uses billing data from a regulated piped water utility in Manila, Philippines.  In this context, almost all households are connected to piped water and in any given month, one in five households is at least 30 days delinquent.\footnote{Piped water access is from the 2010 Census of Population and Housing, and delinquency figures are from water company data described in more detail below.}  At the same time, households have low access to formal credit with only 3.9\% having credit cards and 18.7\% having bank accounts.\footnote{Statistics are from the Philippines 2014 Consumer Finance Survey.}  The estimation results are consistent with this descriptive evidence, producing a large predicted interest rate for borrowing from standard assets.  
%  This strategy also maps bill payment frequencies into estimates of month-to-month income variance and observed consumption patterns into estimates of household water demand.  Don't EVEN SAY THAT!!! 




% is supported by descriptive evidence showing large rates of temporary disconnections in response to disconnection visits.
% Descriptive evidence finds that a large share of households disconnect temporarily when the  

%% NO I then combine this model with a regulatory framework to
% The structural estimation provides a framework for estimating credit constraints paints .  This approach builds on recent structural 

%from a regulated piped water utility in Manila, Philippines to recover measures of credit constraints 

% Households in Manila have low access to formal credit with only 3.9\% having credit cards and 18.7\% having bank accounts.\footnote{Statistics are from the Philippines 2014 Consumer Finance Survey.}  At the same time, 96\% of households are connected to piped water where over \input{tables/share_del}\unskip\% of households are over 30 days delinquent in any given month.\footnote{Piped water access is from the 2010 Census of Population and Housing, and delinquency figures are from water company data described in more detail below.}  


% This paper investigates the welfare effects of counterfactual billing policies in the context of a regulated piped water utility in Manila, Philippines.  
...
% When households permanently disconnect from service, they often leave large outstanding balances, which are passed on to current users through higher prices.



% incorporates delayed payments for goods into household consumption and savings decisions in order to evaluate the welfare effects of counterfactual billing policies.  I build a dynamic model where each period, households have the option of borrowing through a costly asset as well as through postponing payments for a good.  Low interest rates on postponed payments induce households to overconsume this good.  


% The structural estimation uses billing data from a large piped water utility in Manila, Philippines

%\footnote{\cite{banerjee2007economic} find that shopkeepers account for 37\% of all loans to poor households in Udaipur, India while 23\% come from relatives, 18\% from money lenders, and 6\% from cooperatives or banks.}  

% what about the microfinance focus?  ( dont wanna go to far in that comparison tho...)  % old decent phrase : aim to reduce payment flexibility to ensure that utilities recover their production costs. 

% Instead of promoting credit through public utilities, r

% while also dampening household demand
% which outweigh reductions in demand from consumers as a result of prepaid metering in South Africa.\footnote{\cite{jack2015pay} also argue that prepaid meters may help savings constrained consumers by allowing households to pay small increments each month.} 

 
% focused on promoting microfinance organizations and formal banking opportunities while encouraging  

% on ways to improve revenue collection for large providers while also promoting the development of microfinance organizations as well as formal finance 



% by reading meters and distributing bills, utilities maintain constant contact with their consumers, which lowers the transaction costs of offering credit.

% By tolerating nonpayment,

% Second, by reading meters and distributing bills, utilities maintain constant contact with their consumers, which lowers the transaction costs of offering credit.  Finally, 



% While the bulk of the public policy and economics literature has focused on , little research has examined the role 

% Public utilities are especially well-suited to offer microloans for a variety of reasons.  (1) they are regulated which ensures they cover costs.

% Delaying payment for goods and services provides another potential mechanism to allow households to smooth consumption.  Despite 

% achieve that has received little attention in existing economic research and policy debates.  Despite , this channel of .  

% Outstanding balances for credit cards versus average outstanding balances for delinquent water users.

% ITS ALL ABOUT CONSUMPTION SMOOTHING!!

% Morduch, Besley, Townsend

% 1. households in developing countries want to smooth their consumption

% 2. public utilities help households smooth their consumption by delaying bill payment. 

% 3. They are able to do this because they can prevent households from consuming in the future because they have monopolies.

% 4. Policymakers want to end this practice.


% 2. while formal credit is lacking and frequent migration prevents roscas from forming, almost all households are connected to public utilities, which may provide an important source of credit

% 2. urban vs. rural comparison ?  hard to do in cities because people move a lot (roscas are hard to establish, people are not formally banked, etc.)



% want to think about later credit as smoothing credit ( credit cards can be smoothed !! , don't look at home loans, etc. )

% ITS NOT ABOUT INVESTMENTS IN BIG THINGS (like getting big loans for)
% Access to credit can generate economic growth by allowing households to smooth their consumption.   

% Yet, poor credit access persists throughout the developing world as a result of many market failures (\cite{morduch1999microfinance}, \cite{demirguc2018global}).  Households are often unable to provide sufficient collateral (\cite{besley1995group}), weak transport infrastructure increases the transaction costs of issuing and monitoring loans (\cite{jack2014risk}), and poor information environments raise the costs of screening high-risk borrowers (\cite{stiglitz1990peer}).  

%Public utilities may provide important sources of low-cost credit that overcome many of these market failures.  

% Expanding access to credit has been a primary goal to generate economic growth especially in developing countries.  Yet, poor credit access persists throughout the developing world as a result of many market failures (\cite{morduch1999microfinance}, \cite{demirguc2018global}).  Households are often unable to provide sufficient collateral (\cite{besley1995group}), weak transport infrastructure increases the transaction costs of issuing and monitoring loans (\cite{jack2014risk}), and poor information environments raise the costs of screening high-risk borrowers (\cite{stiglitz1990peer}).  Public utilities may provide important sources of low-cost credit that overcome many of these market failures.  

% Public utilities often allow households to delay bill payments for many months, which act as sizable microloans given that households typically spend 4\% of their income on electricity and 1-2\% of income on water.  First, the threat of service disconnection allows utilities to enforce payment.  Second, by reading meters and distributing bills, utilities maintain constant contact with their consumers, which lowers the transaction costs of offering credit.  Finally, utilities are also able to spread credit risk by raising prices for all consumers to cover delinquency from some consumers.   

% For example, households in Manila, Philippines anecdotally emphasize the negative social stigma associated with being unable to pay for water and electricity.  

% Despite these advantages, policymakers have been aggressively promoting the adoption of prepaid metering technologies, which eliminate any credit that may be provided by public utilities.  Prepaid meters require upfront payment before releasing any services (ie. water, electricity, or gas).  This technology has expanded rapidly among water and electricity utilities in Sub-Saharan Africa as a way to ensure cost recovery for utilities (\cite{heymans2014limits}, \cite{jack2016charging}).  \cite{jack2016charging} find that increases in firm profitability outweigh reductions in demand from consumers as a result of prepaid metering in South Africa.\footnote{\cite{jack2015pay} also argue that prepaid meters may help savings constrained consumers by allowing households to pay small increments each month.}  

% Data constraints have prevented previous studies from examining how prepaid meters may 
...
%In practice, this counterfactual setting may correspond to a policy of strict enforcement by the water company (ie. monthly delinquency visits). 
...
\end{align*}
%With prepaid meters, the water company would also no longer need to conduct delinquency visits.  
...
%Moving to prepaid meters may also affect total water consumption, which would in turn affect the water company's costs of water production.  The water company reports a marginal cost of 5 PhP/m3.  
...
Column (4) of Table~\ref{table:counter} includes the results of the prepaid metering counterfactual.  To cover the much higher costs of prepaid meters, the price intercept, $p_1$, increases by around \input{tables/p_increase_per}\unskip\% as indicated by the third row.  Households also no longer receive a disconnection rebate of \input{tables/delinquency_cost}PhP per month.  With higher prices and without water credit, households lower their consumption by \input{tables/c_h_pp_drop}\unskip\% under prepaid metering compared to the current setting in Column (1).  In order to be indifferent between the current setting and a world with prepaid metering, households would need to receive \input{tables/U_pp_abs}PhP/month in compensation.  Taken together, these results provide suggestive evidence that prepaid metering would be welfare reducing in this context.
