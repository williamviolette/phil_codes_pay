...
% \input{tables/descriptives_usage_groups}
% OLD \\
% \input{tables/descriptives_inc_groups}
\bottomrule \\ [-.8em]
% \multicolumn{5}{l}{Total Households: \input{tables/total_hhs_stayers}  Obs. per Household: \input{tables/obs_per_hh_all} Total Obs.: \input{tables/total_obs_all}} \\
...
\toprule
 ...
 \toprule
\input{tables/table_lfs_analysis}
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
...
% 1) \textbf{No Disconnection} \hspace{.2em} \input{tables/tcd_share_pay}\unskip\% immediately pay their outstanding bill to avoid disconnection.

% 2) \textbf{Permanent Disconnection} \hspace{.2em} \input{tables/tcd_share_dc}\unskip\% disconnect within two months of a visit and never reconnect during the sample.  Disconnection typically involves placing a metal lock on the water meter that stops any flow.  These households may have already planned to change dwellings, switch to a new water source, or leave Manila at the time that they received a delinquency visit.  Households that permanently disconnect often leave large outstanding balances that are never repaid during the sample and average \input{tables/out_bal}PhP.  These unpaid balances impose a financial burden on the utility.  % In total, by the end of the sample, \% of households remain disconnected. 

% 3) \textbf{Temporary Disconnection} \hspace{.2em} \input{tables/tcd_share_rec}\unskip\% disconnect within two months of a visit and reconnect later in the sample.  In order to reconnect, households must pay a small, one-time fee of 200 PhP on top of settling any outstanding balances.\footnote{In the connection survey, households report being reconnected \input{tables/days_rec_average}days on average after full payment.}  While households often negotiate for additional time to pay their outstanding balances,  only \input{tables/enough_time}\unskip\% of households report having ``enough time'' to make their payments.\footnote{\input{tables/days_pay_under_30}\unskip\% of households report agreeing to pay within 30 days and the average grace period is \input{tables/days_pay_average}days.}


% For households who fail to pay, 

% \begin{enumerate}
% 	\item \textbf{Immediate Reconnection}:  In response to a visit, \input{tables/tcd_share_pay}\unskip\% of households pay their outstanding bill immediately to avoid disconnection.
% 	\item \textbf{Permanent Disconnection}: \input{tables/tcd_share_dc}\unskip\% disconnect within two months of a visit and never reconnect during the sample.  These households may have already planned to change dwellings, switch to a new water source, or leave Manila at the time that they received a delinquency visit.  Households that permanently disconnect often leave large outstanding balances that are never repaid during the sample averaging \input{tables/out_bal}PhP. % In total, by the end of the sample, \% of households remain disconnected. 
% 	\item \textbf{Temporary Disconnection}:  \input{tables/tcd_share_rec}\unskip\% disconnect within two months of a visit and reconnect later. While households often negotiate for additional time to pay their outstanding balances,  only \input{tables/enough_time}\unskip\% of households report having ``enough time'' to make their payments.\footnote{\input{tables/days_pay_under_30}\unskip\% of households report agreeing to pay within 30 days and the average grace period is \input{tables/days_pay_average}days.}
% \end{enumerate}  
...
In each period, households face a budget constraint as follows
% \begin{align}\label{eq:bc}
% \begin{split}
% x_t \, + \, p(w_t) w_t \, &= \, y_t  \, + \, \Big[ A_t  - \dfrac{A_{t+1}}{1+r^{a}} \, \Big] \, + \, \Big[ \,B_t -  \dfrac{B_{t+1}}{1+r^{b}} \, \Big] \\
...
% \begin{align}\label{eq:bc}
% \begin{split}
% x_t \, + \, p(w_t) w_t \, &= \, y_t \, + \,  A_t  - \dfrac{A_{t+1}}{1+r^{a}_{t}}  \, +
% \begin{cases}
% B_t -  \dfrac{B_{t+1}}{1+r^{b}_{t}} - f & \text{ if } D_{t+1} = 1 \\
% B_t -  \dfrac{B_{t+1}}{1+r^{b}_{t}} & \text{ if }  \\  \, - D_{t+1} f 
% \end{cases}
% % I_t \, &= \, D_{t+1} + (1-c_t) (1-D_t) (1-D_{t+1}) \\
% % B_t& -  p(w_t) w_t (1-D_{t+1})(1+r_b) \leq B_{t+1} \leq 0 
% \end{split}
...
% J = 
% \begin{cases}
% B_{t+1}  & \text{ if } v_t = 0 \text{ and } C_{t}=1 \text{ and } C_{t+1}=1 \text{ and } B_{t+1}\geq B_t -  pw_t , \\
% 0   & \text{ if } v_t = 1 \text{ and } C_{t}=1 \text{ and } C_{t+1}=1 , \\
% B_{t+1} - f  & \text{ if } C_{t+1}=0 \text{ and } B_{t+1} \geq B_t,    \\
% 0   & \text{ if }  C_{t}=0  \text{ and } C_{t+1}=1, \\
% 0 & \text{ else }
% \end{cases}
% \end{align*}
\begin{align}\label{eq:bc}
...
% \Big[ B_t  \, - C_{t+1}C_t (1-v_t)B_{t+1}\, - \, (1-C_{t+1})(B_{t+1}-f)\Big]
% x_t \, + \, p w_t \, &= \, y_t \, + \,  A_t  - \dfrac{A_{t+1}}{1+r}  \, + \, B_t \,  - \, J_{t+1} \\
% p &= p_1 + p_2 w_t
% J &= 
% \begin{cases}
% B_{t+1}  & \text{ if } v_t = 0 \text{ and } C_{t}=1 \text{ and } C_{t+1}=1 \text{ and } B_{t+1}\geq B_t -  pw_t , \\
...
% Additionally, households must satisfy the following borrowing constraint each month, which ensures that water debt must be less than this month's bill and unpaid bills from previous months
% \begin{align}\label{eq:borrowingconstraint}
% B_{t+1} &\geq B_t -pw_tC_{t+1}
% \end{align}
...
\begin{align*}
\begin{split}
% V_t(X_t,z_t) = max_{w_t,x_{t}} \,\,\, u(w_{t},x_{t}) + (1+\delta)^{-1} \Big[ \pi \lambda V(X_{t+1}|v_{t}=0,y_t=(1+\theta)\bar{y})\, \Big]
V_t(X_{t},s_t) &= max_{w_t,x_{t}} \,\,\, u(w_{t},x_{t}) + (1+\delta)^{-1}  E \Big[\, V(X_{t+1},s_{t+1})\Big] \\
X_t &= [w_t,x_t,A_{t+1},B_{t+1},C_{t+1}] \\
% x_t \, + \, p w_t \, &= \, y_t \, + \, \Big[  A_t  - \dfrac{A_{t+1}}{1+r}  \Big] \, + \, \Big[ B_t  \, - C_{t+1}C_t (1-v_t)B_{t+1}\, - \, (1-C_{t+1})(B_{t+1}-f)\Big]  \\
% B_{t+1} &\geq B_t -pw_tC_{t+1}\\
% p &= p_1 + p_2 w_t
% { \text{max} \atop {X_t} }
\end{split}
...
% Water consumption also has no effect on the borrowing constraint when households are disconnected 
% or when households are disconnected
% , (2) when connected households receive collection attempts ($B_{t+1}=0$), or (3) when households are disconnected ($B_{t+1}=0$).
% Households can accumulate water debt in months where they are connected, $C_t=1$, choose to remain connected, $C_{t+1}$, and do not receive a visit, $v_t=0$.  


% For example, households may choose to wash clothes at home instead of at a laundromat or drink water instead of other beverages in months when they are credit constrained.  
% When the borrowing constraint binds and when households are connected and do not receive collection attempts, then households need to use water in excess of $w_t^{*}$ to satisfy their demand for borrowing through unpaid water bills. 
...
% In this model, households first move to Manila and begin consuming piped water.  As households receive positive income shocks, $(1+\theta)\overline{y}$, they accumulate precautionary savings ($A_{t+1}>0$) to insure against future negative income shocks.  In these months, households simply consume water according to their Cobb-Douglas preferences and pay their bills ($B_{t+1}=0$).

% When households receive enough negative shocks, $(1-\theta)\overline{y}$, they spend their savings and begin to borrow.  First, households borrow by not paying their current water bill ($B_{t+1}=-pw_t$) because they face zero interest rates on unpaid bills.  Each successive month, households can increase this debt by continuing to consume water without paying.

% Further negative income shocks may induce households to borrow beyond their current water bill at which point they face a trade-off: borrowing from the standard asset, $A_{t+1}$ at interest rate $r_h$, or further increasing their water consumption ($\overline{w}_t>w_t^{*}$) to expand the size of their unpaid water bill.  For example, instead of taking out a costly loan from a moneylender, households may choose to wash clothes at home instead of at a laundromat or drink water instead of other beverages while leaving their water bills unpaid.  As substitution toward piped water becomes increasingly costly, households can turn to borrowing from the standard asset.

% With probability, $\lambda$, the utility conducts a collection attempt, which confronts indebted households with a choice: pay their debts or disconnect and use an alternative water source.  Since the alternative water source involves an additional fixed cost each month, households with small debts simply pay their debts either by taking out a costly loan or by lowering their consumption.  Households with large debts decide to disconnect in order to avoid taking out a large loan or experiencing a sharp drop in consumption.  Once disconnected, households accumulate savings to eventually pay their debts and reconnect to service.

% Just before leaving Manila, households have an incentive to accumulate large unpaid bills since the utility is unable to collect on these bills.  In these months, households may become more likely to disconnect in response to collection attempts because they only face the fixed costs of an alternative water source for a short period before leaving Manila.





 % do not have incentives to save indefinitely or consume exactly the mean of their incomes each period.  
% \textbf{Step 1:} Given each possible choice of assets, $A_{t+1}$ and $B_{t+1}$, households maximize utility over water, $w_t$, and other goods, $x_t$, subject to their budget constraint.  Let $Y_t$ equal total income net of all assets.  When households do not want to borrow substantially through unpaid water bills (ie. $B_{t+1}$ is small), then households reach an interior solution given that $p=p_1+p_2w_t$ with optimal consumption equaling $w_t^{*} =  \frac{(p_1^2 + 4 \alpha(2 - \alpha) p_2 Y_t)^{.5} - p_1}{2 p_2 (2-\alpha)}$.

% When households want to borrow substantially through unpaid water bills while they are connected (ie. $B_{t+1}$ is large), then households may need to use water in excess of $w_t^{*}$.  For example, households may choose to wash clothes at home instead of at a laundromat or drink water instead of other beverages in months where households are credit constrained.  In these cases, households need to consume enough water so that their current bills and all unpaid bills satisfy their current demand for water borrowing such that $-pw_t + B_t \leq B_{t+1}$.  Let $\overline{w}_t$ denote water consumption when this expression just holds meaning that $\overline{w}_t = \frac{(p_1^2 - 4p_2 (B_{t+1}-B_{t}))^{.5} - p_1}{2 p_2}$.  Setting $w^{*}_t=\overline{w}_t$ describes the threshold where optimal consumption is just able to cover demand for water borrowing.  Substituting $Y_t =  \overline{Y}_t-(B_t-B_{t+1})$, the threshold level of borrowing, $\overline{B}_{t+1}$, is equal to $B_t + \frac{p_1}{8p_2} ( p_1 + (p_1 + \frac{8 \alpha p_2 \overline{Y}_t}{(1-\alpha)})^{.5}) -\frac{\alpha \overline{Y}_t}{2(1-\alpha)}$.  Therefore, when $B_{t+1} \geq \overline{B}_{t+1}$ (ie. less negative), then households consume $w_t^{*}$ but when $B_{t+1} < \overline{B}_{t+1}$ (ie. more negative), then households consume $\overline{w}_t$.  These choices fully characterize household utility given all possible choices of assets.

% \textbf{Step 2:}  Given optimal consumption choices of $w_t$ and $x_t$, households then make asset and connection choices to maximize $V_t(.)$ in all states, $s_t$.  $V_t(.)$ can be solved by working backwards from period $T$ where $A_T = 0$.  Since   

% $pw_{t}^{*} = B_{t+1} - B_t$.   Let $L=B_{t+1} - B_t$ denote the unpaid water bill.  
% substitute consumption of all other goods to water consumption just to create large unpaid water bills.  When 
%  require substantial borrowing through delayed bill payments such that $B_{t+1}$ is very negative.  To satisfy these choices, households may need to consume additional water beyond   to create a large enough bill to fund their 
% Since households consume for a finite number of months, the value function $V_t(.)$ can be solved with backward induction.  In the last period, $T$, households maximize their consumption given that they have to pay debts from their standard asset, $A_t$, but not their water bill, $B_t$.  Given $V_T(.)$, households can then solve for $V_{T-1}$ iterating backwards.  


% % determined by their current income as well as current and future assets, which are taken as given for this step.  When given asset levels require borrowing through unpaid water bills, households face an additional constraint: they need to consume enough water this period to fund their water borrowing target.\todo[color=green]{Need more details again}  For example, households may drink tap water instead of soft drinks or other beverages in months where they are especially credit constrained.  Formally, this situation occurs when equation (\ref{eq:borrowconstraint}) is binding. 
...
% \\
% s.t.& \\
% x_t & \, + \, p w_t \, = \, y_t \, - D_{t+1} f  \, + \, S_t \, - \, S_{t+1} \\
% -p& w_t (1-D_{t+1}) \leq B_{t+1}-B_t \leq 0  \\
% X_t &= [w_t,x_t,A_{t},B_{t},D_{t}] \\
% z_t &= [y_t,c_t]
% \end{split} 
% \end{align}
...
% We also assume that households cannot use their water bill to save  $\frac{B_{t+1}}{1+r_b}\leq0$
...
% % \begin{align}
% % I_t = (1-D_{t+1}) + (1-c_t) (1-D_t) (1-D_{t+1})
...
% This framework results in four possible states depending on whether income, $y_t$, is high or low and whether households receive a delinquency visit given by the indicator variable, $v_t$ which equals 1 with probability, $\lambda$ and 0 with probability $1-\lambda$ where $\lambda \in (0,1)$.  Assuming that the probabilities of reaching each state are uncorrelated with each other yields the following transition matrix between states
...
Households that do not share their piped water connection in the 2015 Family Income and Expenditure Survey have an average income of \input{tables/y_avg}PhP.  


% , which may overestimate income because  or underestimate the income 

% This method may overestimate average household income because the estimation excludes shared water connections, which tend to used by poorer households.  At the same time, this approximation may underestimate household income to the extent that the Family Income and Expenditure Survey includes households that are unconnected to piped water and also tend to be significantly poorer.  The 2010 Census of Population and Housing finds around 4 to 5\% of households are unconnected to piped water (\cite{wjv}). 
...
The last term needed for estimation is the risk of receiving a delinquency visit each month.  The risk of delinquency visits is calculated for the sample of stayer households who are over 31 days delinquent on their bills.\todo[color=red]{i thought they had a 60 day grace period? that means that probability of visit is zero from 31 to 60 days, and some positive number after 60. what happens if you use 60 instead?} 
...
% To measure prices, the non-linear tariff structure is approximated by a linear function of monthly usage with a fixed price $p_1$ equals 
...
% $\delta$, preferences for water consumption, and the relative cost of alternative water sources.  
...
Table~\ref{table:calibratedparam} describes parameters that are assumed or calibrated prior to estimation.  The monthly savings interest rate is calibrated to the prevailing interest rate in the Philippines over this time period.  The estimation assumes a monthly discount rate of 2\%, which implies an annual discount rate of 26.8\% and falls in the range of recent structural and experimental estimates.\footnote{\cite{andreoni2012estimating} estimate rates between 25\% and 35\% in an experimental setting and confirm exponential discounting.  \cite{laibson2007estimating} use a similar consumption-savings structural approach and recover a discount rate of around 15\%.  \cite{gourinchas2002consumption} use a similar structural approach finding a lower discount rate of around 5\%.}  Since this discount rate is assumed to exceed the savings interest rate, households are able to solve for a well-defined value function.
 ...
 % The last term needed for estimation is the risk of receiving a delinquency visit each month, which is calculated for the sample of stayers.
...
\midrule
Compensating Variation (PhP) &   &  & \input{tables/U_ppe}  & \input{tables/U_pp} \\[.2em]
 						     & T1 &  & \input{tables/U_ppe_t1}  & \input{tables/U_pp_t1} \\
							 & T2 &  & \input{tables/U_ppe_t2}  & \input{tables/U_pp_t2} \\
							 & T3 &  & \input{tables/U_ppe_t3}  & \input{tables/U_pp_t3} \\[.5em]
Mean Usage (m3) 			 &   & \input{tables/c_h}  & \input{tables/c_ppe} & \input{tables/c_pp} \\[.2em]
							 & T1 & \input{tables/c_h_t1}  & \input{tables/c_ppe_t1} & \input{tables/c_pp_t1} \\
							 & T2 & \input{tables/c_h_t2}  & \input{tables/c_ppe_t2} & \input{tables/c_pp_t2} \\
							 & T3 & \input{tables/c_h_t3}  & \input{tables/c_ppe_t3} & \input{tables/c_pp_t3} \\
 &         &              &  \\
 % Water Borrowing Interest Rate & 0\% & 100\% & 100\% & ---  \\
%Adjustments to stay revenue neutral  & & \\
Price Intercept (PhP/m3) & & \input{tables/p_int2} &  \input{tables/p_int_ppe2} & \input{tables/p_int_pp2} \\

Credit Supply Costs (PhP) & & \input{tables/total_costs}  & 0 & 0 \\
...
Additional Metering Cost (PhP) & & 0  & 0 & \input{tables/ppinst} \\
