...
Thank you for the opportunity to revise and resubmit this paper and for the thoughtful suggestions.  Please find responses below, and please let me know if you have any questions.  Please also find attached a version with changes highlighted in blue.
...
\item \refquote{Please do your best to complete the welfare comparison by modeling prepaid smoothing strategies.}
  \begin{refpoints}
  \item  The revised version addresses the two concerns raised by Referee 1.  First, to address the concern that prepayment may help avoid higher tariff blocks, a new robustness exercise replaces the increasing block tariff with a single marginal price and finds similar welfare results (p.~\pageref{paper-par:linearrobust}, \parref{par:linearrobust}; p.~\pageref{paper-par:linearpricesappendix}, \parref{par:linearpricesappendix}).\footnote{Constant marginal prices are likely realistic given that increasing block tariffs are difficult to implement with prepaid meters (Jack and Smith [2020]).}  Second, to address the concern that prepayment may avoid extra hassle costs of disconnection enforcement, the counterfactual clarifies its assumption that the technology itself eliminates the need for disconnection enforcement (consistent with how prepaid meters stop water flow automatically when credits run out) (p.~\pageref{paper-par:prepaidexplain}, \parref{par:prepaidexplain}).
  
  \item The revised version also notes how the model provides a channel for prepaid smoothing (p.~\pageref{paper-par:prepaidsmoothing}, \parref{par:prepaidsmoothing}): households can save at zero interest through the standard asset, which is functionally equivalent in the model to accumulating prepaid Pesos on a meter (which could then be converted to water units at the time of consumption).
  \end{refpoints}
...
\item \refquote{Where price follows an inclining block tariff, as it does in this paper's setting in Manila, additional welfare improvements for households are accessible through prepaid metering, but not postpaid metering. Prepaid metered households are able to minimise their total expenditure on the utility over the period they are connected by purchasing their average monthly consumption\ldots This strategy minimises the total number of litres bought at the higher marginal price blocks of the inclining block schedule over the period that the household is connected.}
  \begin{refpoints}
  \item Helpful observation.  A robustness exercise replaces the increasing block tariff with a single marginal price and finds similar welfare effects (p.~\pageref{paper-par:linearrobust}, \parref{par:linearrobust}; Appendix~\ref{paper-appendix:linearprices}, p.~\pageref{paper-par:linearpricesappendix}, \parref{par:linearpricesappendix}).  This counterfactual would be consistent with prepaid metering being easier to implement with a single marginal price (likely because prepaid meters may make it difficult to monitor/implement increasing block tariffs as Jack and Smith [2020] note).  It also looks like, at least in South Africa, people rarely accumulate units across months with prepaid meters even with increasing block tariffs (Jack and Smith, 2020).  
  \item This version also adds clarification about how the standard asset may provide a way for the model to at least capture prepaid smoothing to some extent (p.~\pageref{paper-par:prepaidsmoothing}, \parref{par:prepaidsmoothing}).
  % \item Finally, I wanted to note that in order to focus on the income smoothing/payment aspect, the model abstracts away from variation in water demand for other reasons (ie. shifts in preferences over time, etc.).    
  \end{refpoints}
...
\item \refquote{Just by purchasing ahead, households reduce the risk of receiving demand letters and disconnection notices, as well as reducing the risk of being disconnected if they plan to stay in the utility's service area. Demand letters and disconnection notices reduce utility and so do actual disconnections in the author's model\ldots Avoiding these improves welfare, all else equal.}
  \begin{refpoints}
  \item Interesting thought, and this would likely be an important concern if prepaid meters required manual disconnection; however, the technologies often stop water flow automatically as soon as the meter reaches zero (which would eliminate the need for warnings and disconnections).  This assumption is now made explicit in the revised discussion of prepaid metering (p.~\pageref{paper-par:prepaidexplain}, \parref{par:prepaidexplain}).
  \end{refpoints}
...
\item \refquote{Given the research question, in my opinion, the paper would be a better fit for a development economics journal.}
  \begin{refpoints}
  \item Thank you for raising this.  The revised Introduction now includes a new sentence (p.~\pageref{paper-par:iocite}, \parref{par:iocite}) connecting the paper to an IO literature on utility regulation.  A central question in the IO of regulated utilities is how pricing and enforcement policies distribute costs across heterogeneous consumers given that utilities are natural monopolies subject to cost-recovery constraints.  This paper contributes by showing that payment policy---specifically, how much delinquency a utility tolerates---is an important but overlooked dimension of regulatory design, with connections to work on the consequences of treating utility services as implicit entitlements (Burgess et al.\ 2020) and on how disconnection and penalty policies distribute costs across households (Cicala 2021).
  \end{refpoints}
\end{refpoints}
...
\item \refquote{Access to clean water is a public health issue in developing countries. If the water company takes on dual roles, providing loans and water, it may face difficulties making the necessary investments to supply clean water to the entire population\ldots the paper currently does not account for the water company's investment decisions. In my opinion, it is likely that the current revenue is too low to support optimal investment.}
  \begin{refpoints}
  \item A new paragraph in the Introduction (p.~\pageref{paper-par:manilacontext}, \parref{par:manilacontext}) discusses the Manila context, which includes high-capacity enforcement and relatively large investments in service quality.  This paragraph further discusses the extent to which this context would generalize to other settings---likely those making similarly large investments in water infrastructure.  It also cites Wu and Malaluan [2008], which discusses the public-private partnership regulatory structure in Manila where prices are set to recover costs including investments.  
  \item The conclusion notes the limitation that the model does not capture how tolerating arrears may affect utility investment decisions---an important channel in settings with weaker regulatory oversight, as documented by McRae (2015) for Colombia.  In Manila, cost-recovery pricing and relatively strong regulatory capacity help mitigate this concern.
  \end{refpoints}
...
\item \refquote{If the government wants to provide interest-free loans, why do it via the water utility company? Why not create another entity that provides loans or build an institutional framework that allows the market to do so? In my opinion, for the counterfactual policy analysis, this implies that we shouldn't consider only an epsilon improvement, but also consider what is optimal.}
  \begin{refpoints}
  \item Helpful idea to strengthen the counterfactual logic.  The revised paper calculates how much borrowing costs would need to fall to leave households indifferent to prepaid meters (p.~\pageref{paper-par:microcreditoffset}, \parref{par:microcreditoffset}), providing a benchmark for external credit provision.  The revised counterfactual policy space now spans from eliminating implicit credit entirely (prepaid metering) to penalizing delinquency (late fees and interest, p.~\pageref{paper-par:latepenaltysection}, \parref{par:latepenaltysection}) to expanding it substantially (reduced enforcement).
  \end{refpoints}
