...
The counterfactual exercises also search for the welfare-maximizing level of delinquency enforcement.  Rather than considering a single alternative enforcement rate, this exercise sweeps over a range of enforcement intensities and identifies the optimal warning rate under revenue neutrality.  At the optimum, households substantially increase unpaid balances, indicating more consumption smoothing.  Despite larger unpaid bills, optimal enforcement actually results in a price reduction because of lower enforcement costs and because of greater revenue raised from households substituting toward piped water consumption to increase their borrowing with unpaid bills.\footnote{For example in months where households are especially credit constrained, they may choose to wash clothes at home instead of at a laundromat or drink water instead of other beverages.}  This result suggests that less enforcement would more than pay for itself.  Overall, optimal enforcement is predicted to improve social welfare by \input{tables/cv_comp_hf_b}PhP per household each month.  Given 2.4 million piped water using households in Metro Manila, this estimate implies substantial welfare gains each year.
...
\toprule
 & (1)    & (2)          & (3)   \\
 & Current & Prepaid Metering & Optimal Enforcement   \\
\midrule
Warning Rate ($\lambda$) & \input{tables/vrate_reg_b}& \input{tables/vrate_nl_b}  & \input{tables/vrate_hf_b}  \\
...
\subsection{Optimal Enforcement}
% decreases costs to households of maintaining unpaid water bills as a source of credit.
The next counterfactual searches for the welfare-maximizing level of delinquency enforcement.  Rather than considering only a single alternative enforcement rate, this exercise sweeps over a grid of monthly warning probabilities, $\lambda$, ranging from 2\% to 40\%, computing the revenue-neutral compensating variation at each level.  The optimal enforcement rate, reported in Column~(3) of Tables~\ref{table:countercosts} and~\ref{table:counter}, is \input{tables/vrate_hf_b}\unskip.  At this rate, compensating variation in Column (3) of Table~\ref{table:counter} indicates that households would tolerate a decrease in their monthly incomes of up to \input{tables/cv_comp_hf_b}PhP and still be willing to switch to the optimal enforcement policy.

This welfare gain comes from two channels.  First, less enforcement decreases the costs to households of using unpaid water bills as a source of credit.  Comparing columns (1) and (3) of Table~\ref{table:counter}, households substantially increase their average unpaid balances while also reducing borrowing from standard assets.  Second, less enforcement leads households to increase their water consumption in order to have large unpaid bills as a source of credit.  Greater water consumption increases utility revenue net of pumping costs.  On net, Column (3) of Table~\ref{table:countercosts} indicates that the utility is able to cover its costs at a lower marginal price of \input{tables/p1_hf_b}PhP, which also improves household welfare.
...
The positive welfare effects of reducing enforcement appear robust to alternative assumptions on household preferences.  Appendix Table~\ref{table:robustestimates} provides the compensating variation at the optimal enforcement rate under high and low discount rates and levels of risk aversion.  High discount rates and levels of risk aversion both increase household demand for consumption smoothing, which magnify the welfare benefits of reduced enforcement.
...
\hspace{1em}Optimal Enforcement & \input{tables/cv_comp_hf_b} & \input{tables/cv_comp_hf_bhigh} & \input{tables/cv_comp_hf_blow} & \input{tables/cv_comp_hf_chigh} & \input{tables/cv_comp_hf_clow}    \\
