...
\parmark\label{par:latepenaltyintro}\added{Counterfactual late-payment penalties and interest charges on unpaid bills both substantially reduce unpaid balances while leaving households roughly indifferent to the current equilibrium.  Even under these policies, households remain better off than under prepaid metering, consistent with households deriving value from unpaid bills as additional income insurance.}
...
\parmark\label{par:crosscountry}\added{These payment and disconnection patterns are broadly comparable to other large utilities in both middle and high-income countries.  Service shutoffs are often permitted as a debt recovery tool in most high and middle income countries (\cite{marques2023water}).  For example in South Africa, 20\% of Cape Town electicity consumers were observed disconnected for at least one month likely due to nonpayment (\cite{jack2020charging}).\footnote{Water consumers near Pretoria, South Africa often experienced service restrictions due to non-payment (\cite{szabo2015reducing})} Likewise, among consumers of two large utilities in the United States, around 4\% were served disconnection notices, 20\% received late fees, and 1\% were disconnected due to nonpayment (\cite{cicala2021incidence}). Like in Manila, late fees for water utilities are often heavily regulated with caps ranging from 1.5\% of monthly unpaid balances in New York State up to 11\% in Jerusalem (\cite{nycrr_11_15} and \cite{shavit_krause_polemio_2024}).   Other cities in the Philippines are permitted to charge penalties of around 10\% of unpaid balances.\footnote{See \cite{mnwd_guidelines_bills_payments} and \cite{ppcwater_ur_billing}. } }
