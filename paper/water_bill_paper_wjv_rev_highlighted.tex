...
\parmark\label{par:microcreditoffset}\added{The counterfactual further calculates that households would be indifferent to having prepaid meters if the monthly interest rate of the standard asset were also lowered from from 9.5\% to 7\%.}\footnote{See Table~\ref{table:calibratedparam} for the list of calibrated parameters.}  \added{This result suggests that pairing prepaid meters with microloan programs or other credit policies could help offset the consumer welfare effects of prepaid meters.}
...
\added{The last counterfactuals consider using penalty fees to reducing billing delinquency. For example, other water providers in the Philippines impose late penalties of around 10\% of total unpaid balances.}\footnote{See \cite{mnwd_guidelines_bills_payments} and \cite{ppcwater_ur_billing}.}
