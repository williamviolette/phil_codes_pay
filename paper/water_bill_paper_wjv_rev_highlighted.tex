...
\newcounter{parnum}
\newcommand{\parmark}{\refstepcounter{parnum}\marginnote{\footnotesize\textcolor{blue}{\P\theparnum}}}
...
\begin{titlepage} 
\title{Microcredit from Delaying Bill Payments\\[1em]{\normalsize\textcolor{blue}{Changes highlighted in blue and numbered in the margins}}}
\author{\\[3em]
...
\parmark\label{par:microcreditoffset}\added{The counterfactual further calculates that households would be indifferent to having prepaid meters if the monthly interest rate of the standard asset were also lowered from 9.5\% to 7\%.}\footnote{See Table~\ref{table:calibratedparam} for the list of calibrated parameters.}  \added{This result suggests that pairing prepaid meters with microloan programs or other credit policies could help offset the consumer welfare effects of prepaid meters.}
...
\added{The last counterfactuals consider using penalty fees to reduce billing delinquency. For example, other water providers in the Philippines impose late penalties of around 10\% of total unpaid balances.}\footnote{See \cite{mnwd_guidelines_bills_payments} and \cite{ppcwater_ur_billing}.}
...
\begin{table}[H]
\centering
\caption{\added{Household Outcomes under Counterfactual Policies with Single Marginal Price}}\label{table:counter_linp}
\resizebox{\columnwidth}{!}{%
\begin{threeparttable}
\begin{tabular}{@{}l*{1}{EEEEE}@{}}
\toprule
