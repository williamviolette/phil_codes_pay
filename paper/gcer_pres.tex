...
% %  & Mean & SD  \\
% % \midrule
% % \input{tables/descriptives_pres_stayers}
% % % \bottomrule
% % \end{tabular}
% % % }
% % \end{table}
...
% \vspace{.8mm}
% \item Sample has \input{tables/total_hhs_stayers}HHs with \input{tables/obs_per_hh_stayers}obs per HH for \input{tables/total_obs_stayers}total obs 
% \end{itemize}
...
%\toprule
Water preference & $\alpha $ &\input{tables/est_alpha} (\input{tables/est_sd_alpha}\unskip)  & {\footnotesize   Avg usage   } \\[.1em]
Income shock size  & $\theta $ &  \input{tables/est_theta} (\input{tables/est_sd_theta}\unskip)  & {\footnotesize  Avg unpaid bills } \\[.1em]
Cost of other water  &  $f$ & \input{tables/est_fc} (34.3\unskip) &  {\footnotesize  \% Disc. 1-2 months post visit } \\[.1em]
Borrowing rate & $r_h$ & \input{tables/est_irate}  (\input{tables/est_sd_irate}\unskip)    & \% {\footnotesize  Disc. 1-2 months post visit } \\
