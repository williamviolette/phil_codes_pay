...
clear;
syms x w y p rl rh l alpha A Ap W Wp
...
assume(alpha>0 & alpha<1)
    ...
    % assets (pre) + usage loan (pre)
BC = ( ( y + A + W ) - ( ( Ap + ((p*w)/(1+rl)) ) /(1+rh) ) + ((p*w)/(1+rl)) + ((W)/(1+rl))     -   (p*w+x) ) 
...
steps=10
matlabFunction(simplify(wse,'IgnoreAnalyticConstraints',true,'Steps',steps),'File','w_b_2s.m')
matlabFunction(simplify(vse,'IgnoreAnalyticConstraints',true,'Steps',steps),'File','v_b_2s.m')
...
steps=10
matlabFunction(simplify(wse,'IgnoreAnalyticConstraints',true,'Steps',steps),'File','w_b2_2s.m')
matlabFunction(simplify(vse,'IgnoreAnalyticConstraints',true,'Steps',steps),'File','v_b2_2s.m')
