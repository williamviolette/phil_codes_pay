...
format long g
    ...
    % given :  r_lend , r_water, r_high ,  shock  ,   inc shock, k,  alpha , beta_up , Y , p1, p2 ,pd,  n ,   curve, r_slope, waterlend,
...
inc_t  =  1  ;  %%% which inc to estimate1
                                                                  ...
                                                                  % k
given=       [  r_lend    0      r_lend      0        y_cv(1)       0      .071    .035     y_avg(1)  p1(1)  p2(1)  300   n  2.6   r_slope(1) 0; ...
                r_lend    0      r_lend      0        y_cv(2)       0      .044    .014     y_avg(2)  p1(1)  p2(1)  190   n  1.5  r_slope(2) 0; ...
                r_lend    0      r_lend      0        y_cv(3)       0      .0271   .018     y_avg(3)  p1(1)  p2(1)  210   n  1.5  r_slope(3) 0  ];
...
tic
[est_mom,~,controls,~,~,A1,B1]=dc_obj_k(given(inc_t,:),prob,nA,sigA,Alb(inc_t),Aub(inc_t),nB,sigB,Blb(inc_t),nD,chain,s,int_size,refinement);
toc
...
mean(  ( (p1(1)+controls(:,1).*p2(1)).*controls(:,1) ).*  )
...
% mean(controls(:,2)<0)
    ...
    if short_est==1
        ...
        obj = @(a1)dc_objopt_k(a1,given(inc_t,:),data(:,inc_t),option,option_moments_est,weights,prob,nA,sigA,Alb(inc_t),Aub(inc_t),nB,sigB,Blb(inc_t),nD,chain,s,int_size,refinement);
                    disp ' old obj: ' 
                    obj(ag)
                    disp ' '
                    disp 'pattern search ... '
    ...
    if full_est==1
        for i =1:3
            ...
            ag = given(i,option);    
            obj = @(a1)dc_objopt_k(a1,given(i,:),data(:,i),option,option_moments_est,weights,prob,nA,sigA,Alb(i),Aub(i),nB,sigB,Blb(i),nD,chain,s,int_size,refinement);
