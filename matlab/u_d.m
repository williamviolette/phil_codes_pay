function [util,w] = u_d(L,debt,alpha,p1,p2,y,lambda)

if debt==1
    L_cut = cut_d(alpha,p1,p2,y);

    vb = v_b_d(L,alpha,p1,p2,y);
    vb(isinf(vb))=-1000000;
    
    util  = v_reg_d(L,alpha,p1,p2,y).*(L>=L_cut) + ...
          vb.*(L<L_cut);

    w   = w_reg_d(L,alpha,p1,p2,y).*(L>=L_cut) + ...  %%% less negative!
          w_b_d(L,p1,p2).*(L<L_cut);   %%% more negative !
else
    util  = v_reg_d(L,alpha,p1,p2,y);
    w   = w_reg_d(L,alpha,p1,p2,y);
end
