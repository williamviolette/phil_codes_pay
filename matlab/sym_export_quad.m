...
% lan  =   x - (1/(2*alpha))*(w - k )^2  + l*BC

lan  =   x - (1/2)*(w-alpha)^2  + l*BC
...
simplify(ws)
wt=eval(subs(ws,[p1,p2,y,alpha,A,Ap,L,k],[15,.2,30000,30,100,100,100,0]))
...
matlabFunction(wse,'File','w_reg_quad.m')
matlabFunction(vse,'File','v_reg_quad.m')
...
matlabFunction(simplify(cut_point3,'IgnoreAnalyticConstraints',true,'Steps',steps),'File','cut_quad.m')
...
veq_opt = simplify(xeq_opt - (1/2)*(weq_opt-alpha)^2 );
...
matlabFunction(simplify(weq_opt,'IgnoreAnalyticConstraints',true,'Steps',steps),'File','w_b_quad.m')
matlabFunction(simplify(veq_opt,'IgnoreAnalyticConstraints',true,'Steps',steps),'File','v_b_quad.m')
