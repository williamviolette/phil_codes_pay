function    [h,util,sim,nA,nB,A,B] = dc_obj_k(given,prob,nA,sigA,Alb,Aub,nB,sigB,Blb,nD,chain,s,int_size,refinement)
...
%             Vnow  = mV(Imark,chain(II),ii_alt);
        ...
        [u1,u2,u3,u4,...
                 w1,w2,w3,w4] = ...
             gen_curve_k(Athis,Bthis,Dthis,Ap,Bp,Dp,r_high,r_slope,r_lend,r_water,water_lending,Y_high,Y_low,p1,p2,p1d,p2d,pd,alpha,k,lambda_high,lambda_low,curve);
        u_full = [u1 u2 u3 u4];
        u      = u_full(chain(II));
%         [~,~,~,~,...
%                  w1,w2,w3,w4] = ...
...
%            med(w_debt_m(dd_m~=1 & ind_m<=Acc-tm)); ...
if nargout>1
