...
syms x w Y p1 p2 r A l alpha Ap L
...
simplify(ws)
wt=eval(subs(ws,[p1,p2,Y,alpha,A,Ap,L],[15,.2,30000,.02,100,100,100]))
...
fileID = fopen(strcat(cd_dir,'wsel.tex'),'w');
fprintf(fileID,'%500s\n',latex(wse));
fclose(fileID);
...
fileID = fopen(strcat(cd_dir,'vsel.tex'),'w');
fprintf(fileID,'%500s\n',latex(vse));
fclose(fileID);
...
fileID = fopen(strcat(cd_dir,'cut_point3l.tex'),'w');
...
fileID = fopen(strcat(cd_dir,'weq_optl.tex'),'w');
fprintf(fileID,'%500s\n',latex(simplify(weq_opt,'IgnoreAnalyticConstraints',true,'Steps',steps)));
fclose(fileID);

fileID = fopen(strcat(cd_dir,'veq_optl.tex'),'w');
