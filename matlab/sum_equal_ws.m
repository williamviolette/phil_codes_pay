...
w1 = ((p1^2 + alpha*(2-alpha)*4*(y)*p2)^.5 - p1)/(2*p2*(2-alpha))

w2 = ((p1^2 - 4*p2*(l))^.5 - p1)/2*p2

ww = w1-w2

[sol1,sol2,sol3] = solve(ww,l,'ReturnConditions',true)
...
% BC = ( y - L -   (p*w+x) ) 
% 
% % lan  =  ( (x ).^(1-alpha) ).*( (w).^(alpha) ) + l*BC
% 
% lan  =  ( (x ).^(1-alpha) ).*( (w - k).^(alpha) ) + l*BC
% 
...
% 
% 
% vse = ( (xse-k).^(1-alpha) ).*( (wse+k).^(alpha) )
% 
% eval(subs(vse,[p1,p2,y,alpha,A,Ap,L,k],[15,.2,30000,.02,100,100,100,1]))
% 
...
% 
% matlabFunction(wse,'File','w_reg_inc.m')
% matlabFunction(vse,'File','v_reg_inc.m')
% 
% 
% %%% cut point?
...
% 
% matlabFunction(simplify(cut_point3,'IgnoreAnalyticConstraints',true,'Steps',steps),'File','cut_inc.m')
% 
...
% % fclose(fileID);
% 
% matlabFunction(simplify(weq_opt,'IgnoreAnalyticConstraints',true,'Steps',steps),'File','w_b_inc.m')
% matlabFunction(simplify(veq_opt,'IgnoreAnalyticConstraints',true,'Steps',steps),'File','v_b_inc.m')
% 
